...
Utilizing the dataset generated above, our goal is to build a classification model that will predict whether or not an algal bloom occurs based on concentrations of chlorophyll-a, concentrations of nutrients, and the month of the year (proxy for water temperature). Our first step in building our model was defining our features, or the variables we will be classifying based off of, which are the chlorophyll-a, nutrient, and month variables. Our target feature, or what we're trying to predict, is the bloom variable (whether or not an algal bloom occurs).
```
import pandas as pd
...
First, we utilized a standard 80/20 training/testing split to break our data up and train our model.
```
...
We decided to use the XGBoost package to build our classifier because of its strong performance when handling unbalanced datasets. In our case, our dataset was unbalanced because there were considerably more non-observances of algal blooms (or 0s) than observances (signified by 1s). Then, we evaluated the performance of the classifier by generating a classification report and a confusion matrix.
```
...
We also generated a feature importance plot to get an idea of which environmental variables (nutrients, chlorophyll, or month) are the strongest predictors of algal bloom occurrences.
```
...
The classifier shows strong performance, with 94% accuracy in predicting algal bloom occurrences. It achieves perfect precision (1.00) for detecting blooms (class 1), meaning every predicted bloom was indeed a bloom. However, the recall is lower at 0.67, indicating that the model missed some actual bloom events (2 out of 6). For non-bloom cases (class 0), the model performs very well with high recall (1.00) and precision (0.94). Overall, the model is highly precise and reliable, but improving sensitivity to bloom events would enhance its utility.

The confusion matrix for our classifier supports the high performance of our model in detecting algal blooms. It correctly identified 29 bloom events (True Positives) and made no False Positive errors, meaning it never incorrectly predicted a bloom when there wasn’t one. It also correctly identified 4 non-bloom events (True Negatives), though there were 2 False Negatives, where actual blooms were missed. This suggests the model is highly precise with a strong ability to avoid false alarms, though there is slight room for improvement in sensitivity. In developing a classifier with high accuracy, our aim was to create a tool that could be useful for local environmental agencies in the Outer Banks region in forecasting algal bloom events. That being said, having a model like the one we developed highlights the need for more frequent and consistent water quality monitoring in the Pamlico and Albemarle Sounds, as having high quality data to input to the model is what makes it functional. Additionally, being able to forecast algal bloom occurrences is only useful when risk management strategies are put into place to help minimize the harmful effects of algal blooms on fishermen and coastal economies.

Our feature importance plot suggests that chlorophyll-a concentrations are the most important feature in predicting algal bloom occurrences, while month is the least important. This result is expected, as chlorophyll levels typically rise significantly during algal blooms due to increased algal biomass, making it a strong indicator of bloom presence. It also makes sense that month was the least important feature because it's only an indirect proxy for environmental conditions like water temperature and sunlight, which influence algal blooms. Unlike chlorophyll or nutrient concentration, which are direct biological and chemical indicators of bloom activity, month is a coarse approximation that doesn't capture real-time variability.

## LIMITATIONS AND EXTENSIONS

As discussed previously, our main limitation in building our classification model was the lack of chlorophyll-a, nutrient, and water temperature data that overlapped spatially and temporally with algal bloom observation data. Ideally, each algal bloom observation would’ve had a nearby sampling station from which nutrient, chlorophyll-a, and temperature water samples were taken frequently enough that the algal bloom could have associated environmental variables to build a classification model from. Ultimately, we could not justify building a model off of samples that were taken more than 1 year before or after an observation was recorded. Additionally, water temperature data was taken at fewer sampling stations across the study region, and if we were to incorporate this data, often the nearest algal bloom observation to a temperature sampling location was too far away to reasonably use. Aggregating nutrient and chlorophyll-a concentrations by mean and across several years was also not ideal, as this does not account for fluctuations over the years – for example, if there has been an increase in anthropogenic nutrient inputs to the Pamlico and Albemarle Sounds in recent years, this is not captured. To make our model more robust, additional environmental variables that are typically associated with algal blooms such as dissolved oxygen concentrations, water column stratification, rainfall/runoff, etc. should also be incorporated as features.

In terms of quantifying the risk posed to commercial fisheries in the region, we ideally would have used landings data taken at a smaller spatial and temporal resolution (for example, landings by county per month across 2019-2025). This could have allowed us to define a stronger relationship between algal bloom occurrences with commercial fisheries landings in the Pamlico and Albemarle Sounds. Ultimately, taking the year component out of the nutrient and chlorophyll data to make our model more reasonable in terms of predicting algal bloom occurrences made our analysis of financial risk for commercial fisheries in the study area much more difficult, since we could only recover commercial fisheries landings data on a yearly scale.
